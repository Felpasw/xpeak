...
- ✅ **Phase 2 — Application skeleton**
  ...
  - Phoenix 1.7 API + Postgres, Next.js 15 + Capacitor,
    docker-compose, health endpoint, first tests on both sides.
...
- 📋 **Phase 6 — Check-in media**
  ...
  - Storage layer via a Phoenix-side wrapper (e.g., Waffle or a
    plain `ExAws.S3` adapter) — provider TBD in `README.md` §10.
...
- 📋 **Phase 9 — Editable XP & level configuration** (from
  ...
  - Minimal admin surface: IEx console + Kaffy or LiveDashboard-based
    admin (or a small custom LiveView admin) + audit log.
...
- 📋 **Phase 14 — Global rankings & discovery** (from `ideas.md`
  ...
  - Storage strategy: snapshot job into `rankings_snapshots` +
    ETS/Cachex (or Redis when we add it) cache for top-N.
...
- 📋 **Phase 21 — Metrics dashboard & yearly recap** (from
  ...
  - Always-on dashboard: totals, category breakdown, streak
    ...
    level.
  - "Year in XPeak" story-style recap generated by an Oban job
