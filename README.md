...
- Media & asset storage: **Cloudinary** (`CloudinaryDotNet` SDK) — handles avatars, banners, frames, category/medal/trophy artwork and check-in photo/video. Direct client uploads via unsigned/signed preset, on-the-fly transformations (`w_128,c_fill,f_auto,q_auto`) and global CDN out of the box.
...
- **CheckInMedia** — id, check_in_id, kind (photo|video), cloudinary_public_id, order
...
- **Media storage:** ✅ **Cloudinary** (locked). Free tier for MVP; upgrade path when we hit the credit limit.
