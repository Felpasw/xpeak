...
- **TDD is mandatory across ALL phases.** No production code without a failing test first. Backend with ExUnit, frontend with Vitest/Testing Library, E2E with Playwright or Detox (TBD).
...
### Backend
- **Elixir + Phoenix 1.7** (API mode: no HTML/assets/LiveView by default; Phoenix Channels stay available for realtime features later).
- Postgres as the primary database, accessed via Ecto.
- **Oban** for background jobs (Postgres-backed, no Redis required at MVP).
- Media storage: TBD provider (S3/R2/GCS) — the wrapper picked when Phase 6 lands.
- Auth: Guardian (JWT) + GuardianDb (revocation) + Ueberauth (Google OAuth).
- Tests: ExUnit + ExMachina + Mox for external integrations.
- Style: `mix format` + Credo (+ Dialyxir for opportunistic type checks).

### Infra
- Monorepo with **pnpm workspaces** (JS) and a separate directory for the Phoenix app.
- CI: GitHub Actions with parallel jobs (lint, test, build) covering both sides.
- Deploy: TBD (Fly.io, Gigalixir, or self-hosted Kamal — decided when we reach that phase).
...
- **Auth provider:** own email+password + JWT (Guardian), OAuth (Google/Apple), magic link?
...
- **API contract:** OpenAPI generated by the Phoenix API with codegen to the front, or GraphQL (Absinthe), or manually typed REST?
